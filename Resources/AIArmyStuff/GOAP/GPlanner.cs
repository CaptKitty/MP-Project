    ...
    private bool BuildGraph(Node parent, List<Node> leaves, List<GAction> usableActions, Dictionary<string, int> goal) {
        ...
        foreach (GAction action in usableActions) {
            ...
            if (action.IsAhievableGiven(parent.state)) {
                ...
                Node node = new Node(parent, parent.cost + action.GrabCost(), currentState, action);
                ...
                foreach (var res in currentState)
                {
                    //Debug.Log(res.Key);
                }
                ////Debug.Log(goal.Key + " + " + currentState.Key);
                if (GoalAchieved(goal, currentState)) {
                    ...
                    //Debug.Log("Attempt 2 with:" + usableActions.Count + "Options");
