...
public class SimpleRelay : MonoBehaviour
{
    ...
    {
        Debug.Log("Player - Joining host allocation using join code.");
        Debug.LogError(JoinCodesText.text);

        try
        {
            Debug.Log("joinCode: " + joinCode);
            var joinAllocation = await RelayService.Instance.JoinAllocationAsync(JoinCodesText.text);
            playerAllocationId = joinAllocation.AllocationId;
            Debug.Log("Player Allocation ID: " + playerAllocationId);
            SceneManager.LoadSceneAsync("SampleScene", LoadSceneMode.Additive);
        }
        ...
        {
            Debug.LogError(joinCode + " vs " + JoinCodesText.text + ex.Message + "\n" + ex.StackTrace);
        }
